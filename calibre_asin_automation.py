...
class CalibreASINAutomation:
    ...
    def __init__(self, library_path=None):
        ...
        try:
            ...
            print(
                "✓ Enhanced ASIN Lookup Service initialized with localization support"
            )
        except ImportError:
    ...
    def create_asin_custom_column(self):
        ...
        try:
            cmd = self.get_base_cmd() + [
                ...
                "text",
                "--display",
                '{"description": "Amazon ASIN für Goodreads Integration"}',
            ]
    ...
    def set_asin_for_book(self, book_id, asin):
        ...
        """
        try:
            cmd = self.get_base_cmd() + ["set_custom", "asin", str(book_id), asin]
            ...
            if result.returncode == 0:
                ...
                print(
                    f"✗ Fehler beim Setzen der ASIN für Buch {book_id}: {result.stderr}"
                )
                return False
        ...
        for book in books_without_asin:
            ...
            if not dry_run:
                ...
                if book_file_path and self.localization_extractor:
                    print(
                        f"  Verwende Lokalisierung für: {os.path.basename(book_file_path)}"
                    )
    ...
    def export_library_asin_status(self, output_file=None):
        ...
        try:
            cmd = self.get_base_cmd() + [
                "list",
                "--fields",
                "id,title,authors,#asin,identifiers",
            ]
            result = subprocess.run(cmd, capture_output=True, text=True)
            ...
            if result.returncode == 0:
                ...
                print(
                    f"  Ohne ASIN: {library_status['statistics']['books_without_asin']}"
                )
...
def main():
    ...
    while True:
        ...
        if choice == "1":
            ...
            automation.process_library_automatic_asin(
                max_books=max_books, dry_run=False
            )
            ...
            if existing_files:
                ...
                automation.process_book_files_direct_asin(
                    existing_files, dry_run=dry_run_choice
                )
            else:
