...
class CalibreController:
    ...
    def batch_convert(self, input_dir, output_dir, input_format, output_format, options=None):
        ...
        for filename in os.listdir(directory):
            if any(filename.lower().endswith(ext) for ext in supported_formats):
                ...
                if asin and asin.startswith('B') and len(asin) == 10:
                    ...
                    if self.set_book_metadata(file_path, {'identifier': f'amazon:{asin}'}):
                        ...
                        if convert_result['success']:
                            ...
                            print(f"✗ Konvertierung fehlgeschlagen: {convert_result['stderr']}")
                    else:
...
def main():
    ...
    if os.path.exists(test_dir):
        ...
        if choice == "1":
            ...
            result = controller.add_books_from_directory(test_dir, {
                'duplicates': 'ignore',
                'add-empty': False
            })
