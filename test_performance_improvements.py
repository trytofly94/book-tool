...
def test_sqlite_cache():
    ...
    try:
        ...
        print(f"ℹ️  Cache initialized with {initial_entries} entries (including any migrations)")
        ...
        assert stats['total_entries'] == expected_entries, f"Should have {expected_entries} entries, got {stats['total_entries']}"
        print(f"✓ Cache stats: {stats['total_entries']} entries, {stats['hit_rate']:.1f}% hit rate")
...
def test_rate_limiter():
    ...
    if bucket_status:
        ...
        print(f"✓ Token bucket status: {bucket_status['current_tokens']}/{bucket_status['capacity']} tokens")
...
def test_asin_lookup_integration():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            print(f"✓ Single lookup result: {result.success}, ASIN: {result.asin}, source: {result.source}")
