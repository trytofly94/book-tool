...
class ParallelKFXConverter:
    ...
    def check_kfx_plugin(self):
        ...
        try:
            ...
            result = subprocess.run(['calibre-customize', '-l'], 
                                  capture_output=True, text=True, timeout=10)
    ...
    def parallel_batch_convert(self, input_dir, output_dir=None, input_formats=None, dry_run=False):
        """
        ...
        if dry_run:
            ...
            for candidate in candidates:
                ...
                print(f"Würde konvertieren: {candidate['filename']} → {output_filename}")
            return []
        ...
        print(f"Gesamt-Ausgabe-Größe: {sum(r['file_size'] for r in successful) / 1024 / 1024:.1f} MB")
        
        if failed:
            print(f"\nFehlgeschlagene Konvertierungen:")
            for fail in failed:
                print(f"  ✗ {os.path.basename(fail['input_path'])}: {fail['error'][:100]}...")
    ...
    def convert_library_to_kfx(self, book_filter=None, limit=None, dry_run=False):
        ...
        try:
            ...
            cmd = self.get_base_cmd() + ['list', '--fields', 'id,title,authors,formats,path']
            ...
            for line in lines:
                if line.strip():
                    parts = line.split('\t')
                    if len(parts) >= 5:
                        book_id, title, authors, formats, path = parts[0], parts[1], parts[2], parts[3], parts[4]
                        
                        # Prüfe ob bereits KFX vorhanden
                        if 'AZW3' not in formats.upper() or 'kfx' not in formats.lower():
    ...
    def install_kfx_plugin(self):
        ...
        if install_now == 'j':
            ...
            if os.path.exists(plugin_path):
                try:
                    result = subprocess.run(['calibre-customize', '-a', plugin_path], 
