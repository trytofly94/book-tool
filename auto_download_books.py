...
def search_and_download_series(author, series_name, preferred_format="mobi"):
    ...
    try:
        ...
        mobi_books = [
            book
            for book in results
            if book["format"].upper() == preferred_format.upper()
        ]

        print(
            f"Gefunden: {len(mobi_books)} Bücher im {preferred_format.upper()} Format"
        )
        ...
        for i, book in enumerate(mobi_books):
            ...
            try:
                ...
                if os.path.exists(downloads_path):
                    ...
                    print(
                        f"⚠ Datei nicht im Downloads-Ordner gefunden: {safe_filename}"
                    )
...
def main():
    if len(sys.argv) < 3:
        ...
        print(
            "Beispiel: python auto_download_books.py 'Brandon Sanderson' 'Sturmlicht' mobi"
        )
        sys.exit(1)
