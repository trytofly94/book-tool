...
class SQLiteCacheManager:
    ...
    def _init_database(self):
        ...
        try:
            with self._get_cursor() as cursor:
                ...
                cursor.execute("CREATE INDEX IF NOT EXISTS idx_expires_at ON asin_cache(expires_at)")
                cursor.execute("CREATE INDEX IF NOT EXISTS idx_source ON asin_cache(source)")
                cursor.execute("CREATE INDEX IF NOT EXISTS idx_confidence ON asin_cache(confidence_score)")
                cursor.execute("CREATE INDEX IF NOT EXISTS idx_last_accessed ON asin_cache(last_accessed)")
                ...
                self.logger.debug(f"Initialized SQLite cache database: {self.cache_path}")
            ...
            if json_path.exists():
                try:
                    ...
                    if not isinstance(json_data, dict):
                        self.logger.warning(f"Invalid JSON cache format in {json_path}, skipping")
                        continue
                    ...
                    if entries_to_migrate:
                        ...
                        self.logger.info(f"Migrated {len(entries_to_migrate)} entries from {json_path}")
                    ...
                    self.logger.warning(f"Failed to migrate JSON cache from {json_path}: {e}")
                    continue
        
        if migrated_count > 0:
            self._stats['migrated_entries'] = migrated_count
            self.logger.info(f"Successfully migrated {migrated_count} total cache entries from JSON to SQLite")
    ...
    def cache_asin(self, cache_key: str, asin: str, source: str = "unknown", confidence_score: float = 1.0):
        """
        ...
        try:
            ...
            with self._get_cursor() as cursor:
                cursor.execute("""
                    ...
                    (cache_key, asin, created_at, expires_at, source, confidence_score, access_count, last_accessed)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                """, (cache_key, asin, current_time, expires_at, source, confidence_score, 1, current_time))
            
            self._stats['writes'] += 1
            self.logger.debug(f"Cached ASIN {asin} for key {cache_key} (source: {source}, confidence: {confidence_score})")
    ...
    def cleanup_expired(self) -> int:
        ...
        try:
            ...
            with self._get_cursor() as cursor:
                ...
                cursor.execute("SELECT COUNT(*) FROM asin_cache WHERE expires_at <= ?", (current_time,))
                expired_count = cursor.fetchone()[0]
                
                if expired_count > 0:
                    # Delete expired entries
                    cursor.execute("DELETE FROM asin_cache WHERE expires_at <= ?", (current_time,))
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        try:
            with self._get_cursor() as cursor:
                ...
                cursor.execute("SELECT COUNT(*) FROM asin_cache WHERE expires_at > ?", (time.time(),))
                active_entries = cursor.fetchone()[0]
                
                # Performance statistics
                total_operations = self._stats['hits'] + self._stats['misses']
                hit_rate = (self._stats['hits'] / total_operations * 100) if total_operations > 0 else 0.0
                
                # Source distribution
                cursor.execute("SELECT source, COUNT(*) FROM asin_cache GROUP BY source")
