...
class FormatConverter(LoggerMixin):
    ...
    def __init__(self, config_manager: "ConfigManager"):
        ...
        try:
            ...
            self.logger.debug(
                f"Initialized FormatConverter with max_parallel: {self.max_parallel}, output: {self.output_path}"
            )
        except Exception as e:
            self.logger.warning(
                f"Failed to load conversion config, using defaults: {e}"
            )
            self.max_parallel = 4
        ...
        self.logger.info(
            f"Initialized format converter with output path: {self.output_path}"
        )
    ...
    def validate_kfx_plugin(self) -> bool:
        ...
        try:
            ...
            if re.search(kfx_pattern, result.stdout, re.IGNORECASE):
                ...
                self.logger.warning(
                    "KFX Output plugin not found. Please install it via Calibre Preferences → Plugins"
                )
                return False
            ...
            self.logger.error(
                "calibre-customize command not found. Please install Calibre CLI tools"
            )
            return False
        except Exception as e:
    ...
    def convert_kfx_batch(
        ...
        self.logger.info(
            f"Starting batch KFX conversion of {len(kfx_files)} files to {output_format}"
        )
        ...
        self.logger.info(
            f"Found {len(actual_kfx_files)} actual KFX files out of {len(kfx_files)} provided"
        )
        ...
        if dry_run:
            ...
            for kfx_file in actual_kfx_files:
                ...
                output_file = (
                    output_dir / f"{kfx_file.stem}_from_kfx.{output_format.lower()}"
                )
                ...
                result = ConversionResult(
                    ...
                    conversion_time=0.0,
                    file_size_before=(
                        kfx_file.stat().st_size if kfx_file.exists() else 0
                    ),
                    file_size_after=(
                        kfx_file.stat().st_size if kfx_file.exists() else 0
                    ),  # Estimate
                ...
                if progress_callback:
                    progress_callback(
                        len(kfx_results) / len(actual_kfx_files),
        ...
        for kfx_file in actual_kfx_files:
            ...
            if output_file.exists():
                self.logger.info(
                    f"Skipping {kfx_file.name} - output already exists: {output_file.name}"
                )
                continue
        ...
        self.logger.info(
            f"Processing {len(conversion_jobs)} KFX conversion jobs with {parallel} workers"
        )
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=parallel) as executor:
            # Submit all KFX jobs
            future_to_job = {
                executor.submit(convert_kfx_job, job): job for job in conversion_jobs
            }
            ...
            for future in concurrent.futures.as_completed(future_to_job):
                ...
                try:
                    ...
                    if result.success:
                        size_mb = (
                            result.file_size_after / 1024 / 1024
                            if result.file_size_after
                            else 0
                        )
                        self.logger.info(
                            f"✓ Converted KFX {result.input_file.name} to {output_format.upper()} ({size_mb:.1f} MB)"
                        )
                    else:
                        self.logger.error(
                            f"✗ Failed to convert KFX {result.input_file.name}: {result.error}"
                        )

                        # Provide KFX-specific error guidance
                        if (
                            "plugin" in result.error.lower()
                            or "kfx" in result.error.lower()
                        ):
                            self.logger.error(
                                "  💡 Try installing/updating the KFX Output plugin: calibre-customize -a KFXOutput.zip"
                            )
                    ...
                    self.logger.error(
                        f"✗ Exception converting KFX {job['input_file'].name}: {exc}"
                    )
                    ...
                    if progress_callback:
                        ...
                        progress_callback(
                            progress_percentage,
                            f"KFX Error {completed}/{len(conversion_jobs)}",
                        )
        ...
        if results:  # Only log KFX summary if we actually converted KFX files
            total_kfx_size_before = (
                sum(r.file_size_before or 0 for r in results) / 1024 / 1024
            )
            total_kfx_size_after = (
                sum(r.file_size_after or 0 for r in kfx_successful) / 1024 / 1024
            )
            ...
            self.logger.info(
                f"  📊 Total KFX input size: {total_kfx_size_before:.1f} MB"
            )
            self.logger.info(
                f"  📊 Total KFX output size: {total_kfx_size_after:.1f} MB"
            )
            ...
            if kfx_failed:
                ...
                if len(kfx_failed) > 3:
                    self.logger.warning(
                        f"  ... and {len(kfx_failed) - 3} more KFX failures"
                    )
    ...
    def convert_single(
        ...
        if output_file is None:
            output_file = (
                self.output_path / f"{input_file.stem}.{output_format.lower()}"
            )
        ...
        try:
            ...
            self.logger.debug(
                f"Running conversion command: {' '.join(str(x) for x in cmd)}"
            )
            ...
            if result.returncode == 0:
                ...
                if output_file.exists():
                    ...
                    self.logger.info(
                        f"Successfully converted {input_file.name} to {output_format} ({file_size_after / 1024 / 1024:.1f} MB)"
                    )
                ...
                self.logger.error(
                    f"Conversion failed for {input_file.name}: {error_msg}"
                )
                return ConversionResult(
                    input_file=input_file,
                    output_file=output_file,
    ...
    def convert_batch(
        ...
        self.logger.info(
            f"Starting batch conversion of {len(files)} files to {output_format} (parallel: {parallel})"
        )
            ...
            for file_path in files:
                ...
                if progress_callback:
                    progress_callback(
                        ...
                        f"Preview {len(results)}/{len(files)}",
                    )
            return results
        ...
        for file_path in files:
            ...
            if output_file.exists():
                self.logger.info(
                    f"Skipping {file_path.name} - output already exists: {output_file.name}"
                )
                continue
        ...
        self.logger.info(
            f"Processing {len(conversion_jobs)} conversion jobs with {parallel} workers"
        )
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=parallel) as executor:
            # Submit all jobs
            future_to_job = {
                executor.submit(convert_single_job, job): job for job in conversion_jobs
            }
            ...
            for future in concurrent.futures.as_completed(future_to_job):
                ...
                try:
                    ...
                    if result.success:
                        ...
                        self.logger.info(
                            f"✓ Converted {result.input_file.name} ({size_mb:.1f} MB)"
                        )
                    else:
                        self.logger.error(
                            f"✗ Failed to convert {result.input_file.name}: {result.error}"
                        )
                    ...
                    error_result = ConversionResult(
                        input_file=job["input_file"],
                        output_file=job["output_file"],
                        input_format=self._detect_format(job["input_file"])
                        or BookFormat.EPUB,
                        output_format=BookFormat(output_format.lower()),
                        success=False,
                    ...
                    self.logger.error(
                        f"✗ Exception converting {job['input_file'].name}: {exc}"
                    )
                    ...
                    if progress_callback:
                        ...
                        progress_callback(
                            progress_percentage,
                            f"Error {completed}/{len(conversion_jobs)}",
                        )
    ...
    def find_convertible_files(
        ...
        """
        self.logger.info(
            f"Finding convertible files in {input_dir} (recursive: {recursive})"
        )
        ...
        try:
            ...
            for file_path in file_iterator:
                ...
                if progress_callback and total_checked % 100 == 0:
                    progress_callback(
                        f"Scanned {total_checked} files, found {len(convertible_files)} convertible"
                    )
                ...
                if extension in supported_extensions:
                    ...
                    if (
                        "_kfx" not in file_path.stem.lower()
                        and "_converted" not in file_path.stem.lower()
                    ):
        ...
        self.logger.info(
            f"Found {len(convertible_files)} convertible files in {input_dir}"
        )
    ...
    def get_supported_formats(self):
        ...
        try:
            ...
            self.logger.warning(
                f"Failed to query Calibre formats dynamically, using defaults: {e}"
            )
    ...
    def _query_calibre_formats(self) -> tuple[List, List]:
        ...
        try:
            ...
            result = subprocess.run(
                ["ebook-convert", "--help"], capture_output=True, text=True, timeout=10
            )
