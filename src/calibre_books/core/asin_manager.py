...
class ASINManager(LoggerMixin):
    ...
    def process_books(
        ...
        self.logger.info(f"ASIN processing complete: {successful}/{len(results)} successful")
    ...
    def _process_single_book(self, book: Book, lookup_online: bool) -> ASINLookupResult:
        ...
        Returns:
            ASIN lookup result
        """
        self.logger.debug(f"Processing book: {book.metadata.title} by {book.metadata.author}")
        ...
        if lookup_online and self.lookup_service:
            ...
            if asin:
                ...
                self.logger.info(f"Found ASIN online: {asin} for '{book.metadata.title}'")
    ...
    def remove_asin_from_file(self, file_path: Path) -> bool:
        ...
        try:
            ...
            if current_identifiers:
                cmd = ['ebook-meta', str(file_path), '--identifier'] + current_identifiers
            else:
