...
class FileValidator(LoggerMixin):
    ...
    def validate_directory(
        ...
        """
        self.logger.info(f"Validating files in directory: {directory} (recursive: {recursive})")
        ...
        if parallel and len(files) > 1:
            return self._validate_files_parallel(files, use_cache, progress_callback, max_workers)
        else:
            return self._validate_files_sequential(files, use_cache, progress_callback)
    
    def validate_file(self, file_path: Path, use_cache: bool = True) -> ValidationResult:
        """
    ...
    def _discover_ebook_files(
        ...
        ebook_extensions = {
            '.mobi', '.epub', '.azw', '.azw3', '.pdf', '.txt',
            '.fb2', '.lit', '.pdb', '.rtf', '.docx', '.doc'
        }
        ...
        for file_path in all_files:
            if (file_path.is_file() and 
                file_path.suffix.lower() in ebook_extensions):
    ...
    def save_results(
        ...
        try:
            ...
            output_data = {
                'summary': summary,
                'validation_results': file_results
            }
