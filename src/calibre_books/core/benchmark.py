...
class ASINLookupBenchmark:
    ...
    def run_benchmark(
        ...
        if include_warmup:
            ...
            for i in range(self.warmup_runs):
                self._run_single_benchmark_iteration(books[:min(5, len(books))], parallel_workers, sources)
        ...
        for run_idx in range(self.measurement_runs):
            ...
            run_start = time.time()
            results = self._run_single_benchmark_iteration(books, parallel_workers, sources)
        ...
        # Aggregate results
        return self._aggregate_benchmark_results(run_results, test_name, books, detailed_timing)
    ...
    def _aggregate_benchmark_results(
        ...
        # Cache efficiency
        cache_efficiency = (cache_hits + success_count) / book_count * 100 if book_count > 0 else 0.0
        ...
        errors = [r.error for r in results if not r.success and r.error][:10]  # First 10 errors
    ...
    def compare_benchmarks(
        ...
        def calculate_improvement(baseline_val: float, optimized_val: float, higher_better: bool) -> float:
    ...
    def print_benchmark_summary(self, result: BenchmarkResult):
        ...
        print(f"  Success Rate: {result.success_rate:.1f}% ({result.success_count}/{result.book_count})")
        print(f"  Cache Hit Rate: {result.cache_hit_rate:.1f}% ({result.cache_hits}/{result.book_count})")
        ...
        if result.confidence_distribution and any(result.confidence_distribution.values()):
        ...
        if result.network_requests_made:
            ...
            efficiency = (result.success_count / result.network_requests_made * 100) if result.network_requests_made > 0 else 0
    ...
    def print_benchmark_comparison(self, comparison: BenchmarkComparison):
        ...
        if comparison.improvements:
            print("IMPROVEMENTS:")
            for metric, improvement in sorted(comparison.improvements.items(), key=lambda x: x[1], reverse=True):
                print(f"  {metric.replace('_', ' ').title()}: +{improvement:.1f}%")
            print()
        
        if comparison.regressions:
            print("REGRESSIONS:")
            for metric, regression in sorted(comparison.regressions.items(), key=lambda x: x[1], reverse=True):
        ...
        for attr, name, unit in key_metrics:
            ...
            print(f"{name:<20} {baseline_val:<12.2f} {optimized_val:<12.2f} {change_str:<10}")
