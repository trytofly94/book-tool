...
def kfx(
    ...
    Examples:
        book-tool convert kfx --input-dir ./kfx_books --parallel 4
        book-tool convert kfx --input-dir ./books --output-format epub
        book-tool convert kfx --input-dir ./books --quality high
    """
    ...
    try:
        ...
        if dry_run:
            console.print(
                f"[yellow]DRY RUN: Would convert {len(kfx_files)} KFX files:[/yellow]"
            )
...
def single(
    ...
    Examples:
        book-tool convert single -i book.mobi -f epub
        book-tool convert single -i book.pdf -f txt --no-cover
        book-tool convert single -i book.kfx -o custom_name.epub -f epub
    """
    config = ctx.obj["config"]
    dry_run = ctx.obj["dry_run"]
    ...
    try:
        ...
        if result.success:
            console.print(
                f"[green]Successfully converted to: {result.output_file}[/green]"
            )
...
def batch(
    ...
    Examples:
        book-tool convert batch -i ./books -f epub --recursive
        book-tool convert batch -i ./mobi_books --from-format mobi -f epub
        book-tool convert batch -i ./books -o ./converted --parallel 4 -f pdf
    """
    config = ctx.obj["config"]
    dry_run = ctx.obj["dry_run"]
    ...
    try:
        ...
        if dry_run:
            console.print(
                f"[yellow]DRY RUN: Would convert {len(files_to_convert)} files:[/yellow]"
            )
            console.print(f"  Input directory: {input_dir}")
            console.print(f"  Output directory: {output_dir or 'same as input'}")
