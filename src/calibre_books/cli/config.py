...
def init(
    ...
    try:
        ...
        if interactive:
            ...
            console.print(
                "This wizard will help you set up book-tool for your system.\n"
            )
            ...
            config_data["download"] = {
                ...
                "download_path": Prompt.ask(
                    "Download directory", default="~/Downloads/Books"
                ),
                "librarian_path": Prompt.ask("Librarian CLI path", default="librarian"),
            }
            ...
            config_data["calibre"] = {
                "library_path": Prompt.ask(
                    "Default Calibre library path", default="~/Calibre-Library"
                ),
                "cli_path": Prompt.ask(
            ...
            cache_path = Prompt.ask(
                "ASIN cache directory", default="~/.book-tool/cache"
            )
            ...
            config_data["conversion"] = {
                "max_parallel": int(
                    Prompt.ask("Maximum parallel conversion processes", default="4")
                ),
                "output_path": Prompt.ask(
                    "Conversion output directory", default="~/Converted-Books"
                ),
                "kfx_plugin_required": Confirm.ask(
                    "Require KFX plugin for conversions?", default=True
                ),
            }
            ...
            config_data["logging"] = {
                ...
                "format": Prompt.ask(
                    "Log format", choices=["simple", "detailed"], default="detailed"
                ),
            }
        ...
        try:
            ...
            if not temp_converter.validate_kfx_plugin():
                ...
                console.print(
                    "Install it via: Calibre → Preferences → Plugins → Get new plugins → 'KFX Output'"
                )
                console.print(
                    "See: https://github.com/trytofly94/book-tool#kfx-conversion-prerequisites"
                )
        except Exception:
    ...
    try:
        ...
        if section:
            if section not in config_data:
                console.print(
                    f"[red]Section '{section}' not found in configuration[/red]"
                )
                ctx.exit(1)
...
def edit(
    ...
    try:
        ...
        if not config_path.exists():
            console.print(
                "[red]No configuration file found. Run 'book-tool config init' first.[/red]"
            )
            ctx.exit(1)
    ...
    try:
        ...
        for check in path_checks:
            if check.exists:
                ...
                console.print(
                    f"[yellow]![/yellow] {check.name}: {check.path} (will be created)"
                )
        ...
        if all_deps_ok:
            ...
            console.print(
                "Please install the missing dependencies and run validation again."
            )
