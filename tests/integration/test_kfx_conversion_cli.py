...
class TestKFXConversionCLIIntegration:
    ...
    def test_kfx_conversion_cli_command_with_default_config(self):
        ...
        try:
            ...
            with patch(
                ...
                with (
                    patch("calibre_books.core.downloader.Path.mkdir"),
                    patch(
                        "calibre_books.core.downloader.Path.exists", return_value=True
                    ),
                    ...
                    if result.exit_code != 0:
                        if (
                            "'ConfigManager' object has no attribute 'get'"
                            in result.output
                        ):
                            pytest.fail(
                                f"GitHub Issue #1 regression detected: {result.output}"
                            )
    ...
    def test_kfx_conversion_cli_with_missing_conversion_config(self):
        ...
        try:
            ...
            with patch(
                ...
                assert (
                    "'ConfigManager' object has no attribute 'get'" not in result.output
                )
    ...
    @pytest.mark.skip(
        reason="Legacy test for old ParallelKFXConverter API - needs update for FormatConverter"
    )
            ...
            with patch(
                ...
                with runner.isolated_filesystem():
                    with patch("click.get_current_context") as mock_ctx:
                        ...
                        with patch(
                            "calibre_books.core.file_scanner.FileScanner.scan_directory"
                        ) as mock_scan:
                            mock_scan.return_value = (
                                []
                            )  # No books found is fine for this test
...
class TestKFXConversionCLIErrorHandling:
    ...
    def test_cli_with_nonexistent_config_file(self):
        ...
        result = runner.invoke(
            main, ["--config", str(nonexistent_config), "convert", "kfx", "--help"]
        )
...
class TestKFXConversionConfigManagerFlow:
    ...
    def test_config_manager_data_flow_to_kfx_converter(self):
        ...
        config_data = {
            ...
            "calibre": {"library_path": "~/test-library", "cli_path": "/test/calibre"},
        }
    ...
    @pytest.mark.skip(
        reason="Legacy test for old KFXConverter API - needs update for FormatConverter"
    )
    def test_cli_context_passing_to_converter(self):
        """Test that CLI properly passes ConfigManager to converter."""
        config_file = tempfile.NamedTemporaryFile(mode="w", suffix=".yml", delete=False)
        yaml.dump(
            {"conversion": {"max_parallel": 3}, "calibre": {"library_path": "~/test"}},
            config_file,
        )
        config_file.close()
        ...
        try:
            ...
            with patch("calibre_books.cli.convert.KFXConverter") as mock_kfx_class:
                ...
                def mock_init(config_manager):
                    ...
                    mock_converter.validate_kfx_plugin.return_value = (
                        True  # Mock validation success
                    )
                    return mock_converter
