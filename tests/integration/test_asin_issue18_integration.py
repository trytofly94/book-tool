...
class TestASINLookupIssue18Integration:
    ...
    def test_source_filtering_via_cli(self, mock_service_class):
        ...
        result = self.runner.invoke(lookup, [
            '--book', 'Test Book',
            '--author', 'Test Author',
            '--sources', 'goodreads'  # Should map to google-books
        ], obj=self.create_mock_context())
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for title, author, expected_asin in test_cases:
                ...
                with patch.object(service, '_lookup_via_amazon_search', return_value=expected_asin):
                    ...
                    assert result.success is True, f"Failed lookup for '{title}' by {author}"
    ...
    def test_cli_handles_network_errors_gracefully(self, mock_service_class):
        ...
        mock_service = Mock()
        mock_service_class.return_value = mock_service
        mock_service.lookup_by_title.side_effect = Exception("Network connection failed")
