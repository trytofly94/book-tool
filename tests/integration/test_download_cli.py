...
from calibre_books.core.downloader import DownloadResult, BookRequest
...
class TestDownloadCliCommands:
    ...
    def test_books_command_no_criteria(self, mock_downloader_class):
        ...
        result = self.runner.invoke(books, [], obj=self.create_mock_context())

        assert result.exit_code == 1
        assert (
            "Must specify at least one of --series, --author, or --title"
            in result.output
        )
    ...
    def test_books_command_no_results(self, mock_progress, mock_downloader_class):
        ...
        result = self.runner.invoke(
            books, ["--title", "Nonexistent Book"], obj=self.create_mock_context()
        )
    ...
    def test_books_command_dry_run(self, mock_downloader_class):
        ...
        result = self.runner.invoke(
            books, ["--series", "Test Series", "--format", "epub"], obj=mock_context
        )
    ...
    @patch("calibre_books.cli.download.BookDownloader")
    @patch("calibre_books.cli.download.ProgressManager")
    def test_books_command_exception_handling(
        self, mock_progress, mock_downloader_class
    ):
        ...
        result = self.runner.invoke(
            books, ["--title", "Test Book"], obj=self.create_mock_context()
        )
    ...
    def test_batch_command_dry_run(self, mock_downloader_class):
        ...
        mock_books = [
            ...
            ),  # More than 5 for truncation test
        ]
        mock_downloader.parse_book_list.return_value = mock_books
        ...
        with self.runner.isolated_filesystem():
            ...
            result = self.runner.invoke(
                batch, ["--input-file", "books_list.txt"], obj=mock_context
            )
    ...
    @patch("calibre_books.cli.download.BookDownloader")
    @patch("calibre_books.cli.download.ProgressManager")
    def test_batch_command_exception_handling(
        self, mock_progress, mock_downloader_class
    ):
        """Test batch command exception handling."""
        mock_downloader = Mock()
        mock_downloader_class.return_value = mock_downloader
        mock_downloader.parse_book_list.side_effect = FileNotFoundError(
            "File not found"
        )
    ...
    def test_url_command_success(self, mock_progress, mock_downloader_class):
        ...
        mock_result = DownloadResult(
            "book.epub", "Unknown", Path("/downloads/book.epub"), True, file_size=1024
        )
    ...
    def test_url_command_failure(self, mock_progress, mock_downloader_class):
        ...
        mock_result = DownloadResult(
            "book.epub", "Unknown", None, False, error="Download timed out"
        )
        mock_downloader.download_from_url.return_value = mock_result
    ...
    def test_url_command_dry_run(self, mock_downloader_class):
        ...
        assert result.exit_code == 0
        assert (
            "DRY RUN: Would download from URL: https://example.com/book.epub"
            in result.output
        )
    ...
    @patch("calibre_books.cli.download.BookDownloader")
    @patch("calibre_books.cli.download.ProgressManager")
    def test_url_command_with_custom_options(
        self, mock_progress, mock_downloader_class
    ):
        ...
        mock_result = DownloadResult(
            "custom_book.epub", "Unknown", Path("/custom/custom_book.epub"), True
        )
        mock_downloader.download_from_url.return_value = mock_result
