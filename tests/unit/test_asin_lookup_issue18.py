...
class TestASINLookupIssue18Fixes:
    ...
    def test_verbose_logging_functionality(self):
        ...
        mock_response.content = b'<html><div data-asin="B00TEST123">Test Book</div></html>'
        mock_response.headers = {'Content-Type': 'text/html'}
        mock_response.url = 'https://amazon.com/s?k=test'
        
        with patch('calibre_books.core.asin_lookup.requests.get', return_value=mock_response):
            with patch('time.sleep'):
                ...
                assert asin == "B00TEST123" or asin is None  # Allow either success or failure
    ...
    def test_enhanced_error_reporting(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(service, '_lookup_via_amazon_search', return_value=None) as mock_amazon, \
                 patch.object(service, '_lookup_via_google_books', return_value=None) as mock_google, \
                 patch.object(service, '_lookup_via_openlibrary', return_value=None) as mock_openlibrary:
                
                result = service.lookup_by_title("Nonexistent Book", author="Unknown Author")
    ...
    def test_progress_callback_integration(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(service, '_lookup_via_amazon_search', return_value="B00PROGRESS"):
                result = service.lookup_by_title(
                    "Test Book",
                    author="Test Author",
                ...
                progress_calls = [call[1]['description'] for call in progress_callback.call_args_list]
...
class TestIssue18RealWorldExamples:
    ...
    def test_brandon_sanderson_examples_mock(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for title, author, expected_asin in test_cases:
                with patch.object(service, '_lookup_via_amazon_search', return_value=expected_asin):
                    result = service.lookup_by_title(title, author=author)
                    
                    assert result.success is True, f"Failed to find ASIN for '{title}' by {author}"
                    assert result.asin == expected_asin, f"Wrong ASIN for '{title}': expected {expected_asin}, got {result.asin}"
                    assert result.source == "amazon-search"
    ...
    def test_error_handling_for_nonexistent_books(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(service, '_lookup_via_amazon_search', return_value=None), \
                ...
                result = service.lookup_by_title("Completely Nonexistent Book That Doesn't Exist", 
