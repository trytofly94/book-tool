...
class TestFormatConverterInitialization:
    ...
    def create_mock_config_manager(
        self, conversion_config: Optional[Dict[str, Any]] = None
    ) -> Mock:
...
class TestConvertSingle:
    ...
    def create_test_file(
        self, suffix: str = ".epub", content: bytes = b"test content"
    ) -> Path:
    ...
    def test_convert_single_input_file_not_exists(self):
        ...
        result = converter.convert_single(
            input_file=non_existent_file, output_format="mobi"
        )
    ...
    def test_convert_single_input_path_not_file(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result = converter.convert_single(input_file=dir_path, output_format="mobi")
    ...
    def test_convert_single_unsupported_format(self):
        ...
        try:
            result = converter.convert_single(
                input_file=test_file, output_format="mobi"
            )
    ...
    def test_convert_single_dry_run(self):
        ...
        try:
            result = converter.convert_single(
                input_file=test_file, output_format="mobi", dry_run=True
            )
...
class TestConvertBatch:
    ...
    def test_convert_batch_skip_existing_outputs(self, mock_convert_single):
        ...
        try:
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                results = converter.convert_batch(
                    files=test_files, output_dir=output_dir, output_format="mobi"
                )
    ...
    def test_convert_batch_default_output_dir(self):
        ...
        try:
            results = converter.convert_batch(
                files=test_files, output_format="mobi", dry_run=True
            )
...
class TestConvertKfxBatch:
    ...
    def test_convert_kfx_batch_plugin_not_required(self, mock_validate):
        ...
        try:
            results = converter.convert_kfx_batch(test_files, dry_run=True)
    ...
    def test_convert_kfx_batch_filters_non_kfx_files(self, mock_validate, mock_detect):
        ...
        try:
            results = converter.convert_kfx_batch(all_files, dry_run=True)
    ...
    @patch.object(FormatConverter, "convert_single")
    @patch.object(FormatConverter, "validate_kfx_plugin")
    def test_convert_kfx_batch_skip_existing_outputs(
        self, mock_validate, mock_convert_single
    ):
        ...
        try:
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                results = converter.convert_kfx_batch(
                    test_files, output_dir=output_dir, output_format="epub"
                )
    ...
    @patch.object(FormatConverter, "convert_single")
    @patch.object(FormatConverter, "validate_kfx_plugin")
    def test_convert_kfx_batch_parallel_execution(
        self, mock_validate, mock_convert_single
    ):
...
class TestFindConvertibleFiles:
    ...
    def test_find_convertible_files_recursive_search(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            files_nonrecursive = converter.find_convertible_files(
    ...
    def test_find_convertible_files_source_format_filter(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            epub_files = converter.find_convertible_files(
                dir_path, source_format="epub"
            )
            assert len(epub_files) == 1
            assert epub_file in epub_files

            mobi_files = converter.find_convertible_files(
                dir_path, source_format="mobi"
            )
            assert len(mobi_files) == 1
            assert mobi_file in mobi_files

            # Test with format that has leading dot
            epub_files_dot = converter.find_convertible_files(
                dir_path, source_format=".epub"
            )
    ...
    def test_find_convertible_files_unsupported_source_format(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            files = converter.find_convertible_files(
                dir_path, source_format="unsupported"
            )
...
class TestHelperMethods:
    ...
    def test_detect_format(self):
        ...
        assert (
            converter._detect_format(Path("book.htm")) == BookFormat.HTML
        )  # HTML alias
