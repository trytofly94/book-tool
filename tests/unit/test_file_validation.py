...
class TestValidationResult:
    ...
    def test_add_error_and_warning(self):
        ...
        result = ValidationResult(
            status=ValidationStatus.VALID, file_path=Path("/test.epub")
        )
...
class TestValidationCache:
    ...
    def test_cache_persistence(self, tmp_path):
        ...
        result = ValidationResult(status=ValidationStatus.VALID, file_path=test_file)
...
class TestFileFormatDetection:
    ...
    def test_unknown_format(self, tmp_path):
        ...
        with open(unknown_file, "wb") as f:
            f.write(
                b"\x89\xfe\xdc\xba\x01\x02\x03\x04\x05\x06\x07\x08\x09\x0a"
            )  # Binary data
