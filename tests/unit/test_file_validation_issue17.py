...
class TestFileValidator:
    ...
    def test_file_discovery(self, validator):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Test discovery
            files = validator._discover_ebook_files(tmpdir_path, recursive=False, formats=None)
    ...
    def test_format_filtering(self, validator):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            files = validator._discover_ebook_files(tmpdir_path, recursive=False, formats=['epub'])
            assert len(files) == 1
    ...
    def test_recursive_discovery(self, validator):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Test recursive
            files = validator._discover_ebook_files(tmpdir_path, recursive=True, formats=None)
            assert len(files) == 2
    ...
    def test_validation_summary_generation(self, validator):
        ...
        # Create mock results
        results = [
            ValidationResult(ValidationStatus.VALID, Path("book1.epub"), format_detected='epub'),
            ValidationResult(ValidationStatus.VALID, Path("book2.mobi"), format_detected='mobi'),
            ValidationResult(ValidationStatus.EXTENSION_MISMATCH, Path("fake.epub"), 
                           format_expected='epub', format_detected='docx', 
                           errors=["Extension mismatch"]),
            ValidationResult(ValidationStatus.CORRUPTED, Path("corrupt.mobi"), 
