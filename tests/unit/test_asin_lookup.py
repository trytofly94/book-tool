...
class TestASINLookupService:
    ...
    def test_lookup_by_isbn_direct_success(self, mock_get):
        ...
        mock_response.url = "https://www.amazon.com/dp/B00ZVA3XL6?ref=dp_kindle_redirect"
    ...
    def test_amazon_search_success(self, mock_get):
        ...
        asin = service._lookup_via_amazon_search("The Way of Kings", "Brandon Sanderson")
    ...
    def test_lookup_by_title_with_amazon_success(self, mock_amazon):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            cached_asin = service.cache_manager.get_cached_asin("the way of kings_brandon sanderson")
    ...
    def test_lookup_by_title_no_results(self, mock_openlibrary, mock_google, mock_amazon):
    ...
    @patch.object(ASINLookupService, '_lookup_via_google_books')
    @patch.object(ASINLookupService, '_lookup_via_openlibrary')
    def test_lookup_by_isbn_with_direct_success(self, mock_openlibrary, mock_google, mock_direct):
    ...
    def test_progress_callback(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(service, '_lookup_via_amazon_search') as mock_amazon:
                ...
                assert progress_callback.call_count >= 2  # At least start and trying amazon
    ...
    def test_cache_thread_safety(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            def cache_asins(thread_id):
                for i in range(10):
                    cache_manager.cache_asin(f"thread_{thread_id}_key_{i}", f"asin_{thread_id}_{i}")
