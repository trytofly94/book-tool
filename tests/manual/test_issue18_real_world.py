...
def test_error_cases(verbose=False):
    ...
    error_cases = [
        (
            "Completely Nonexistent Book Title That Should Never Exist",
            "Fake Author Name",
        ),
        ("", ""),  # Empty strings
    ...
    for i, (title, author) in enumerate(error_cases, 1):
        ...
        try:
            ...
            if result.success:
                print(
                    f"⚠ UNEXPECTED SUCCESS: Found ASIN {result.asin} (this may be incorrect)"
                )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Real-world testing for ASIN lookup issue #18 fixes"
    )
    parser.add_argument("--verbose", action="store_true", help="Enable verbose output")
    parser.add_argument(
        "--validation-only",
        action="store_true",
        help="Only run validation tests (no network calls)",
    )
    ...
    try:
        ...
        print(
            f"Issue #18 examples: {successful_lookups}/{len(issue_results)} succeeded"
        )

        expected_failures = sum(
            1 for _, _, result, _ in error_results if result == "expected_failure"
        )
        print(
            f"Error handling: {expected_failures}/{len(error_results)} behaved correctly"
        )
        ...
        for title, author, success, asin, source in issue_results:
            ...
            print(
                f"  {status} '{title}' by {author}: {asin or 'No ASIN'} ({source or 'No source'})"
            )
