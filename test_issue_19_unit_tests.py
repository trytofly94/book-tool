...
class TestLocalizationMetadataExtractor(unittest.TestCase):
    ...
    def test_language_mappings_initialization(self):
        ...
        self.assertEqual(self.extractor.language_mappings['de']['amazon_domain'], 'amazon.de')
        self.assertEqual(self.extractor.language_mappings['fr']['amazon_domain'], 'amazon.fr')
    ...
    def test_guess_language_from_title(self):
        ...
        # Test German titles
        self.assertEqual(self.extractor._guess_language_from_title('Kinder des Nebels'), 'de')
        self.assertEqual(self.extractor._guess_language_from_title('Der Weg der Könige'), 'de')
        self.assertEqual(self.extractor._guess_language_from_title('Ruf der Sterne'), 'de')
        
        # Test French titles
        self.assertEqual(self.extractor._guess_language_from_title('Les enfants de la brume'), 'fr')
        
        # Test English fallback
        self.assertEqual(self.extractor._guess_language_from_title('Mistborn'), 'en')
        self.assertEqual(self.extractor._guess_language_from_title('The Way of Kings'), 'en')
    ...
    def test_extract_from_filename(self):
        ...
        # Test basic pattern
        metadata = self.extractor.extract_from_filename('/path/sanderson_mistborn1_kinder-des-nebels.epub')
        ...
        metadata = self.extractor.extract_from_filename('/path/brandon-sanderson_elantris.epub')
    ...
    def test_find_english_equivalent(self):
        ...
        english = self.extractor._find_english_equivalent('Kinder des Nebels', 'Brandon Sanderson')
        self.assertEqual(english, 'Mistborn')
        
        # Test known German -> English for Stormlight
        english = self.extractor._find_english_equivalent('Der Weg der Könige', 'Brandon Sanderson')
        self.assertEqual(english, 'The Way of Kings')
        
        # Test unknown title
        english = self.extractor._find_english_equivalent('Unknown German Title', 'Some Author')
    ...
    def test_get_localized_search_terms(self):
        ...
        english_terms = [t for t in search_terms if t['strategy'] == 'english_equivalent']
    ...
    def test_is_likely_corrupted(self):
        ...
        self.assertTrue(self.extractor._is_likely_corrupted('/fake/path', bad_zip_error))
    ...
    def test_merge_metadata(self):
        ...
        fallback = {'title': 'Fallback Title', 'author': 'Fallback Author', 'language': 'en'}
...
class TestASINLookupService(unittest.TestCase):
    ...
    def test_validate_asin(self):
        ...
        # Invalid ASINs
        self.assertFalse(self.lookup_service.validate_asin('A001234567'))  # Doesn't start with B
    ...
    def test_lookup_via_google_books(self, mock_get):
        ...
        asin = self.lookup_service.lookup_via_google_books('1234567890', 'Test Title', 'Test Author')
        self.assertEqual(asin, 'B123456789')
    ...
    def test_lookup_multiple_sources_with_localization(self, mock_search_terms, mock_extract):
        ...
        with patch.object(self.lookup_service, 'lookup_with_localized_terms', return_value='B123456789'):
            asin = self.lookup_service.lookup_multiple_sources(file_path='/fake/path.epub')
            self.assertEqual(asin, 'B123456789')
    ...
    def test_lookup_multiple_sources_fallback(self):
        ...
        with patch.object(service_no_localization, 'lookup_standard_sources', return_value='B987654321'):
            asin = service_no_localization.lookup_multiple_sources(title='Test Title', author='Test Author')
...
class TestIntegration(unittest.TestCase):
    ...
    def test_end_to_end_localization_flow(self):
        ...
        localized_primary = next(t for t in search_terms if t['strategy'] == 'localized_primary')
        ...
        english_equivalent = next((t for t in search_terms if t['strategy'] == 'english_equivalent'), None)
    ...
    def test_edge_case_handling(self):
        ...
        is_corrupted = self.extractor._is_likely_corrupted('/fake/path.epub', fake_error)
