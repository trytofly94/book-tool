...
class TestIssue18And19Regression(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with patch.object(
            CalibreASINAutomation, "check_calibre_availability", return_value=True
        ):
    ...
    def test_backward_compatibility_english_books(self):
        ...
        english_equivalent_searches = [
            s for s in search_terms if s["strategy"] == "english_equivalent"
        ]
    ...
    def test_multi_source_lookup_integration(self):
        ...
        result = self.lookup_service.lookup_standard_sources(
            isbn="test_isbn", title="test_title", author="test_author"
        )
    ...
    def test_asin_validation_consistency(self):
        ...
        for asin in valid_asins:
            self.assertTrue(
                self.lookup_service.validate_asin(asin),
    ...
    def test_cross_language_fallback_chains(self):
        ...
        cross_language_terms = [
            s for s in search_terms if s["strategy"].startswith("cross_language")
        ]
    ...
    def test_corrupted_file_handling_with_localization(self):
        ...
        is_corrupted = self.extractor._is_likely_corrupted(
            "/fake/path.epub", corrupted_error
        )
        self.assertTrue(is_corrupted)

        # Should fall back to filename extraction
        fallback_metadata = self.extractor.extract_from_filename(
            "/fake/corrupted_german_book.epub"
        )
    ...
    def test_calibre_integration_compatibility(self):
        ...
        with patch.object(self.automation, "get_book_file_path", return_value=None):
            ...
            with patch.object(
                self.automation.lookup_service,
                "lookup_multiple_sources",
                return_value="B123456789",
            ) as mock_lookup:

                # Simulate processing a book without file path (Issue #18 scenario)
                result = mock_lookup(
                    isbn="1234567890", title="Test Title", author="Test Author"
                )
    ...
    def test_rate_limiting_across_strategies(self):
        ...
        metadata = {"title": "Test Title", "author": "Test Author", "language": "de"}
        ...
        with patch.object(
            self.lookup_service, "lookup_via_amazon_search_localized", return_value=None
        ):
    ...
    def test_search_term_priority_ordering(self):
        ...
        self.assertEqual(
            priorities, sorted(priorities), "Search terms should be ordered by priority"
        )
        ...
        english_equiv_index = next(
            i for i, s in enumerate(strategies) if s == "english_equivalent"
        )
        self.assertLess(
            english_equiv_index, 5, "English equivalent should be high priority"
        )
