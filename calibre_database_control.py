...
class CalibreDatabaseController:
    ...
    def backup_database(self, backup_path=None):
        ...
        if not backup_path:
            ...
            backup_path = os.path.join(
                self.library_path, f"metadata_backup_{timestamp}.db"
            )
    ...
    def get_book_formats(self, book_id):
        ...
        with self.connect() as conn:
            ...
            formats = []
            for row in cursor.fetchall():
                formats.append({"format": row[0], "filename": row[1], "size": row[2]})
    ...
    def export_library_info(self):
        ...
        with self.connect() as conn:
            ...
            info["top_authors"] = [
                {"name": row[0], "books": row[1]} for row in cursor.fetchall()
            ]
