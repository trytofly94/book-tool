...
class ASINLookupService:
    ...
    def __init__(self, cache_file="/tmp/asin_cache.json", rate_limit=2.0):
        ...
        self.localization_extractor = (
            LocalizationMetadataExtractor() if LocalizationMetadataExtractor else None
        )
    ...
    def lookup_multiple_sources(
        self, isbn=None, title=None, author=None, file_path=None
    ):
        """
        ...
        if file_path and self.localization_extractor:
            ...
            search_terms = self.localization_extractor.get_localized_search_terms(
                metadata
            )
    ...
    def lookup_with_localized_terms(self, search_term, isbn=None):
        ...
        try:
            ...
            if asin and self.validate_asin(asin):
                logger.info(
                    f"✓ ASIN found via localized Amazon search ({language}): {asin}"
                )
    ...
    def lookup_standard_sources(self, isbn=None, title=None, author=None):
        ...
        lookup_methods = [
            ...
            (
                "OpenLibrary",
                lambda: self.lookup_via_openlibrary(isbn) if isbn else None,
            ),
        ]
    ...
    def batch_lookup(self, books_data, max_workers=3):
        ...
        def lookup_single(book_data):
            book_id = book_data.get(
                "id", f"{book_data.get('title', '')}_{book_data.get('author', '')}"
            )
...
def test_asin_lookup():
    ...
    print("\n=== Test completed ===")
    print(
        "Note: For full localization testing, ensure the test EPUB file is available."
    )
