...
def test_comprehensive_asin_lookup(test_dir=None):
        ...
        try:
            ...
            if metadata and metadata.get("title"):
                ...
                if metadata.get("language") == "de":
                    ...
                    print(
                        f"  ✓ Metadata: {metadata['title']} ({metadata.get('language', 'unknown')})"
                    )
            ...
            print(f"  ✗ Error: {e}")
            results["detailed_results"].append(
                {"file": book_file, "metadata_extracted": False, "error": str(e)}
            )
    ...
    success_rate_metadata = (
        results["metadata_extracted"] / results["total_books"]
    ) * 100
    success_rate_asin = (
